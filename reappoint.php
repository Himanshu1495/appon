...
$level_check = mysqli_query($connect,"SELECT level FROM ministers WHERE id='$mini_id'");
while ($row=mysqli_fetch_assoc($level_check)) {
	$mini_level = $row['level'];
}
$mini_level = $mini_level+1;

$appointment_check = mysqli_query($connect,"SELECT app_date,slot_value FROM minister_appointment WHERE mini_id='$mini_id' AND level='$mini_level' ORDER BY app_date DESC,slot_value DESC LIMIT 1");
...
getnextslot($time,$app_date,$reason,$location,$no_of_vis,$agenda,$mini_id,$email,$connect,$mini_level);


function getnextslot($time,$app_date,$reason,$location,$no_of_vis,$agenda,$mini_id,$email,$connect,$mini_level){
	$curr_date = $app_date;
	$curr_slot = $time;
	if ($curr_slot<16) {
